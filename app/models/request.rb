class Request < ActiveRecord::Base
	...
	def process_amz_response(mws_connection, response_xml,page_num,sleep_if_error)
		...
		if self.request_type=="ListOrders"
			...
			response_xml.orders.each do |o|
				r = Order.process_amz_order(mws_connection, o,response.id)
				if r.is_a?(Numeric)
					return r
				end
			end
			...
			response_xml.order_items.each do |i|
				Order.process_amz_order_item(i,response.amazon_order_id)
			end
		end
